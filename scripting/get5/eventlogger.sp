...
#define EventLogger_EndEvent(%1) \
    EventLogger_LogEvent(%1, params)
...
public void EventLogger_SidePicked(MatchTeam team, const char[] map, int mapNumber, int side) {
    ...
    set_json_string(params, "map_name", map);
    set_json_int(params, "map_number", mapNumber);
    set_json_string(params, "side", map);
...
public void EventLogger_PlayerDeath(int killer, int victim, bool headshot,
    int assister, int flash_assister, const char[] weapon) {
    EventLogger_StartEvent();
    AddMapData(params);
